...
------------------------
Light_Cannon_Shell init.lua
	-Light Tank Round Entity serverside init
*/
...
function ENT:Initialize()
	...
	util.SpriteTrail(self.Entity, 0, Color(255,255,200,50), false, 3, 1, 2, 1/(15+1)*0.5, "trails/smoke.vmt")
end
	...
	if ent:IsValid() then
		if ent.MyPlayer then
			if ent.MyPlayer:IsValid() then
				ent.MyPlayer:TakeDamage(17, self.Entity:GetOwner(), self.Entity)
			end
		end
	end
	//boom
	self.Entity:EmitSound("weapons/explode4.wav", 125, 105)
	local ed = EffectData()
